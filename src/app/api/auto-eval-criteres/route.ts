...
export async function GET() {
  ...
  const { data, error } = await supabase
    ...
    .select('id, affirmation, ordre, actif')
    .eq('actif', true)
    .order('ordre')
...
export async function PATCH(request: NextRequest) {
  ...
  const { id, affirmation, ordre, actif } = await request.json()
